...
clicked_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	static const char *msg = "post message";
	...
	if (curl) {
		...
		if (response != CURLE_OK) {
			dlog_print(DLOG_INFO, LOG_TAG, "POST FAILED!\n");
		} else {
			dlog_print(DLOG_INFO, LOG_TAG, "POST SUCCESSFUL!\n");
		}
...
widget_instance_create(widget_context_h context, bundle *content, int w, int h, void *user_data)
{
	...
	elm_object_style_set(wid->button, "default");
	evas_object_resize(wid->button, w, h);
	evas_object_move(wid->button, 0, 0);
	evas_object_smart_callback_add(wid->button, "clicked", clicked_cb, user_data);
